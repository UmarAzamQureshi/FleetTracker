...
export class WebSocketManager {
  ...
  constructor(private url: string = `${config.WS_BASE_URL}/ws`) {}
  ...
  private notifyListeners(event: string, data: WebSocketMessage): void {
    ...
    // Also notify specific event listeners based on message type
...
// Utility functions for working with telemetry data
export const telemetryUtils = {
  // Convert GeoJSON features to vehicle data for the dashboard
  convertToVehicleData(features: GeoJSONFeature[]): any[] {
    const deviceMap = new Map<string, any>();

    features.forEach(feature => {
      const { device_id, ts, speed_kmh, heading } = feature.properties;
      ...
      if (!deviceMap.has(device_id)) {
        deviceMap.set(device_id, {
          ...
          fuel: Math.floor(Math.random() * 100), // Mock fuel data
          driver: `Driver ${device_id}`, // Mock driver data
          lat,
          lng: lon,
          lastUpdate: '1 min ago', // Mock update time
          distance: '0 km', // Mock distance
        });
      }
